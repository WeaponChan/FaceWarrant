...
#import "LhkhTagButton.h"
#import "LhkhTag.h"
@interface LhkhTagView()
...
- (void)onTag: (UIButton *)btn {
    if (self.didTapTagAtIndex) {
        self.didTapTagAtIndex([self.subviews indexOfObject: btn],btn.titleLabel.text);
    }
}
...
- (void)addTag: (LhkhTag *)tag {
    NSParameterAssert(tag);
    LhkhTagButton *btn = [LhkhTagButton buttonWithTag: tag];
...
- (void)insertTag: (LhkhTag *)tag atIndex: (NSUInteger)index {
    NSParameterAssert(tag);
