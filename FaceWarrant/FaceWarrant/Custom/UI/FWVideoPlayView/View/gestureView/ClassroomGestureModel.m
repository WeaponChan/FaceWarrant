...
@interface ClassroomGestureModel()<UIGestureRecognizerDelegate>
/*
 * 功能 ： 单击手势
 */
...
@end
@implementation ClassroomGestureModel
    ...
    switch (sender.state) {
        case UIGestureRecognizerStateBegan: {
            if (moveOrientation == ALYPVOrientationUnknow) {
                ...
                if (self.delegate && [self.delegate respondsToSelector:@selector(ClassroomGestureModel:state:moveOrientation:)]) {
                    [self.delegate ClassroomGestureModel:self state:sender.state moveOrientation:moveOrientation];
                }
            }
            break;
        }
...
- (void)gestureRecognizerStateChangedWithPoint:(CGPoint)point view:(UIView*)view savePoint:(CGPoint)savePoint {
    ...
    if(direction == UISwipeGestureRecognizerDirectionLeft){
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(ClassroomGestureModel:brightnessDirection:)]) {
            [self.delegate ClassroomGestureModel:self brightnessDirection:tempDirection];
        }
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(ClassroomGestureModel:volumeDirection:)]) {
            [self.delegate ClassroomGestureModel:self volumeDirection:tempDirection];
        }
    }
}
