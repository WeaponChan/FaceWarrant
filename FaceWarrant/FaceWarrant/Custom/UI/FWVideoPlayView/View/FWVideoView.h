...
@class FWVideoView;
@protocol FWVideoViewDelegate <NSObject>
...
/**
 ...
 @param videoId 视频ID
 @param quality 清晰度
 @param videoDefinition 视频信息
 */
- (void)FWVideoViewDelegatevodPlayerPlaybackAddressExpiredWithVideoId:(NSString *)videoId quality:(AliyunVodPlayerVideoQuality)quality videoDefinition:(NSString*)videoDefinition;

//界面

//点击播放
- (void)onClickedPlayButtonWithClassroomPlayView:(FWVideoView *)playerView;

/**
 * 功能：返回按钮事件
 * 参数：playerView ：ClassroomPlayVie
 */
...
@interface FWVideoView : UIView

@property (nonatomic, weak) id<FWVideoViewDelegate> delegate;
