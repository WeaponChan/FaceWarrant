...
@interface LhkhVoiceRecognitionView()<UIGestureRecognizerDelegate,CircularProgressDelegate,UIAlertViewDelegate>
{
...
@implementation LhkhVoiceRecognitionView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        m_circularProgressBar = [[CircularProgressBar alloc] initWithFrame:CGRectZero type:@"voiceRecognition"];
...
- (void)startClick:(id)sender {
    ...
    [[LhkhIFlyMSCManager shareManager] recognitionBlock:^(NSString *result) {
        ...
        if (self.voiceLab.text != nil || self.voiceLab.text.length != 0 || ![self.voiceLab.text isEqualToString:@""]) {
            if ([self.delegate respondsToSelector:@selector(LhkhVoiceRecognitionViewDelegateWithText:)]) {
                [self.delegate LhkhVoiceRecognitionViewDelegateWithText:self.voiceLab.text];
                self.voiceLab.text= @"";
            }
        }
    } vctype:self.vctype];
