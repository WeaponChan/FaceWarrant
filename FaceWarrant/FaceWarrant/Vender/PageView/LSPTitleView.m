...
//    [self addSubview:self.scrollView];
    ...
    UIView *cview = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.width-40, 40)];
    cview.backgroundColor = Color_White;
...
- (void)setupAddBtn
{
    UIView *addview = [[UIView alloc] initWithFrame:CGRectMake(Screen_W-40, 0, 40, 40)];
    addview.backgroundColor = Color_White;
    [self addSubview:addview];
    
    UIButton *addBtn = [[UIButton alloc] initWithFrame:CGRectMake(10, 10, 20, 20)];
...
- (void)setupTitleLabelFrame{
    ...
    {
//        titleW = (self.frame.size.width-80) / self.titleLabels.count;
...
- (void)setupBottomLine{
    ...
    [self.scrollView addSubview:self.bottomLine];
    self.bottomLine.y = self.bounds.size.height - self.style.bottomLineH;
    self.bottomLine.height = 2;
    CGSize titleSize = [self.titleLabels.firstObject.text sizeWithFont:systemFont(14) constrainedToSize:CGSizeMake(MAXFLOAT, 40)];
    self.bottomLine.width = titleSize.width;
    self.bottomLine.centerX = self.titleLabels.firstObject.centerX;
...
- (void)add
{
    if ([self.delegate respondsToSelector:@selector(titleViewWithTitleViewAddClick:selectedIndex:)]) {
        [self.delegate titleViewWithTitleViewAddClick:self selectedIndex:self.currentIndex];
    }
}
...
//            self.bottomLine.frame = CGRectMake(currentLabel.frame.origin.x + ((currentLabel.frame.size.width - self.bottomLine.frame.size.width) / 2.0), self.bottomLine.frame.origin.y, self.style.bottomLineW, self.bottomLine.frame.size.height);
            ...
            CGSize titleSize = [currentLabel.text sizeWithFont:systemFont(14) constrainedToSize:CGSizeMake(MAXFLOAT, 40)];
            self.bottomLine.width = titleSize.width;
            self.bottomLine.centerX = currentLabel.centerX;
        }];
    }
...
- (void)setTitleWithProgress:(CGFloat)progress sourceIndex:(NSInteger)sourceIndex targetIndex:(NSInteger)targetIndex{
    ...
    sourceLabel.textColor = Color_Black;
    targetLabel.textColor = Color_Theme_Pink;
    [UIView animateWithDuration:0.5 animations:^{
        CGSize titleSize = [targetLabel.text sizeWithFont:systemFont(14) constrainedToSize:CGSizeMake(MAXFLOAT, 40)];
        self.bottomLine.width = titleSize.width;
...
//    NSLog(@"======contentViewDidEndScroll");
    ...
    //计算中间位置的偏移量
    CGFloat offSetX = targetLabel.center.x - (self.bounds.size.width-40) * 0.5;
    if (offSetX < 0) {
        offSetX = 0;
    }
    
    CGFloat maxOffset = self.scrollView.contentSize.width - (self.bounds.size.width-40);
