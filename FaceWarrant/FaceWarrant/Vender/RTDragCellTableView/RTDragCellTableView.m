...
- (void)longPressGestureRecognized:(id)sender{
    ...
    switch (longPressState) {
        case UIGestureRecognizerStateBegan:{  //手势开始，对被选中cell截图，隐藏原cell
            ...
            _moveIndexPath = [self indexPathForRowAtPoint:_fingerLocation];
            
            if (_moveIndexPath) {
                [self cellSelectedAtIndexPath:_moveIndexPath];
            }
            break;
        }
        case UIGestureRecognizerStateChanged:{//点击位置移动，判断手指按住位置是否进入其它indexPath范围，若进入则更新数据源并移动cell
            ...
            //手指按住位置对应的indexPath，可能为nil
            _relocatedIndexPath = [self indexPathForRowAtPoint:_fingerLocation];
            if (_relocatedIndexPath && ![_relocatedIndexPath isEqual:_moveIndexPath]) {
...
- (void)updateDataSource{
    ...
    if ([self nestedArrayCheck:tempArray]) {//是嵌套数组
        if (_moveIndexPath.section == _relocatedIndexPath.section) {//在同一个section内
            [self moveObjectInMutableArray:tempArray[_moveIndexPath.section] fromIndex:_moveIndexPath.row toIndex:_relocatedIndexPath.row];
        }else{                                                          //不在同一个section内
            id originalObj = tempArray[_moveIndexPath.section][_moveIndexPath.item];
            [tempArray[_relocatedIndexPath.section] insertObject:originalObj atIndex:_relocatedIndexPath.item];
            [tempArray[_moveIndexPath.section] removeObjectAtIndex:_moveIndexPath.item];
        }
    }else{                                  //不是嵌套数组
        [self moveObjectInMutableArray:tempArray fromIndex:_moveIndexPath.row toIndex:_relocatedIndexPath.row];
    }
    //将新数组传出外部以更改数据源
...
- (void)cellRelocatedToNewIndexPath:(NSIndexPath *)indexPath{
    ...
    //交换移动cell位置
    DLog(@"_moveIndexPath====%ld indexPath=====%ld ",_moveIndexPath.row,indexPath.row);
    [self moveRowAtIndexPath:_moveIndexPath toIndexPath:indexPath];
 ...
 */
- (void)didEndDraging{
    UITableViewCell *cell = [self cellForRowAtIndexPath:_moveIndexPath];
    cell.hidden = NO;
    cell.alpha = 0;
    [UIView animateWithDuration:0.2 animations:^{
        ...
        DLog(@"end--originalIndexPath-->%ld --_moveIndexPath-->%ld --_relocatedIndexPath-->%ld",self->_originalIndexPath.row,self->_moveIndexPath.row,self->_relocatedIndexPath.row);
        if ([self.delegate respondsToSelector:@selector(cellDidEndMovingInTableView:fromIndex:toIndex:)]) {
            [self.delegate cellDidEndMovingInTableView:self fromIndex:self->_originalIndexPath toIndex:self->_relocatedIndexPath];
        }
        self->_snapshot = nil;
        self->_moveIndexPath = nil;
